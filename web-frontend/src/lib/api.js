const API_BASE_URL =
  import.meta.env.VITE_WEB_BFF_URL || "http://localhost:3104";
...
export async function getMyLogs(token, page = 1, limit = 20) {
  return apiRequest(
    `/api/logs/my-logs?page=${page}&limit=${limit}`,
    { method: "GET" },
    token,
  );
}
...
export async function getAllLogs(token, filters = {}) {
  ...
  return apiRequest(
    `/api/logs/all?${search.toString()}`,
    { method: "GET" },
    token,
  );
}

export async function getClientLogs(token, filters = {}) {
  const search = new URLSearchParams();
  ...
  return apiRequest(
    `/api/logs/clients?${search.toString()}`,
    { method: "GET" },
    token,
  );
}
...
export async function getTransactionHistory(
  ...
  return apiRequest(
    `/api/payments/history?${qs.toString()}`,
    { method: "GET" },
    token,
  );
}
...
export async function listAccounts(token, clientId = null) {
  const qs = clientId ? `?client_id=${encodeURIComponent(clientId)}` : "";
  return apiRequest(`/api/accounts${qs}`, { method: "GET" }, token);
}

export async function createAccount(token, clientId, currency) {
  return apiRequest(
    "/api/accounts",
    {
      method: "POST",
...
export async function updateAccountStatus(token, accountId, status) {
  return apiRequest(
...
export async function getAccountLogs(token, filters = {}) {
  const search = new URLSearchParams();
  Object.entries(filters).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {
      search.set(key, String(value));
  ...
  return apiRequest(
    `/api/logs/accounts?${search.toString()}`,
    { method: "GET" },
    token,
  );
}
