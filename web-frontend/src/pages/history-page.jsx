...
export default function HistoryPage() {
    ...
    const load = async (targetPage = 1) => {
        ...
        try {
            ...
            const res = await getTransactionHistory(token, undefined, PAGE_SIZE, offset);
    ...
    return (
        <>
            <section className="page-intro dashboard-intro">
                ...
                <p>Full ledger of wallet transfers and admin top-ups.</p>
            </section>
            <section className="content-grid single-column">
                <section className="panel compact-panel">
                    ...
                    <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', marginBottom: '0.8rem' }}>
                        <button type="button" className="button-muted" onClick={() => load(page)} disabled={loading}>
                            ↻ Refresh
                        </button>
                        {!loading && <span style={{ fontSize: '0.8rem', color: 'var(--ink-soft)' }}>{total} record{total !== 1 ? 's' : ''}</span>}
                    </div>
