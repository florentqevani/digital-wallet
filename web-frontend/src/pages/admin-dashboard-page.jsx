...
export default function AdminDashboardPage() {
    ...
    const [summary, setSummary] = useState({ totalClientLogs: 0, totalUserLogs: 0, errorCount: 0, successCount: 0 });
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                ...
                if (active) {
                    setSummary(response.summary || { totalClientLogs: 0, totalUserLogs: 0, errorCount: 0, successCount: 0 });
        ...
        <>
            ...
            <section className="content-grid dashboard-grid">
                <section className="panel compact-panel dashboard-summary-panel">
                    ...
                    <p>{isSuperAdmin ? 'Global activity and reliability trends.' : 'A snapshot of recent actions and outcomes.'}</p>
                    {error && <p className="error-text">{error}</p>}
                    <div className="stats-row">
                        <StatCard label="Client Logs" value={summary.totalClientLogs} tone="default" />
                        <StatCard label="User Logs" value={summary.totalUserLogs} tone="default" />
