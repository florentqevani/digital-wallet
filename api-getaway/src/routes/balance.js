...
router.get('/', validateJWT(['client', 'user', 'superadmin']), async (req, res) => {
    try {
        const response = await promisifyGRPC(userClient.GetClientBalance.bind(userClient), {
            client_id: req.user.user_id,
        });
        res.json({ balance: response.balance, currency: response.currency });
    } catch (error) {
        console.error('❌ Get balance error:', error.message);
        res.status(500).json({ balance: 0, currency: 'ALL', message: error.message });
    }
});

// POST /api/balance/add — adds an amount to the authenticated client's balance
router.post('/add', validateJWT(['client', 'user', 'superadmin']), async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(userClient.AddBalance.bind(userClient), {
            client_id: req.user.user_id,
            amount,
        });
