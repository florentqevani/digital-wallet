...
router.post("/credit-request", validateJWT(["client"]), async (req, res) => {
  ...
  if (!payer_email || typeof payer_email !== "string" || !payer_email.includes("@")) {
    return res.status(400).json({ success: false, message: "payer_email must be a valid email address" });
  }
  if (isNaN(amount) || amount <= 0) {
  ...
  try {
    ...
    res.status(502).json({ success: false, message: "Could not create credit request" });
  }
});
...
router.get("/credit-requests", validateJWT(["client"]), async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ success: false, requests: [], total: 0, message: "Could not retrieve credit requests" });
  }
});
...
router.post("/credit-request/:id/respond", validateJWT(["client"]), async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ success: false, message: "Could not respond to credit request" });
  }
});
