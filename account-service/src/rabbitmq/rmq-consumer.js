...
async function processMessage(msg, ch) {
    ...
    ch.ack(msg);
    console.log(`✅ [AccountConsumer] account_id ${account_id} assigned to client ${client_id}`);
}
...
async function startConsumer() {
    while (true) {
        try {
            ...
            conn.on('error', err => console.error('[AccountConsumer] Connection error:', err.message));
            conn.on('close', () => console.warn('[AccountConsumer] Connection closed'));
            ...
            console.error(`[AccountConsumer] ${err.message} — reconnecting in ${RETRY_DELAY}ms`);
