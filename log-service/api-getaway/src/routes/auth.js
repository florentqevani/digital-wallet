...
router.post('/register', authLimiter, async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(authClient.RegisterClient.bind(authClient), {
            email,
            password,
            name: name || '',
        });
...
router.post('/login-client', authLimiter, async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(authClient.LoginClient.bind(authClient), {
            email,
            password,
        });
...
router.post('/login-user', authLimiter, async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(authClient.LoginUser.bind(authClient), {
            email,
            password,
        });
...
router.post('/register-user', validateJWT(['superadmin']), async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(userClient.RegisterUser.bind(userClient), {
            email,
            password,
            name: name || '',
            role: role || 'user',
...
router.post('/validate', async (req, res) => {
    try {
        ...
        const response = await promisifyGRPC(authClient.ValidateToken.bind(authClient), {
            token,
        });
