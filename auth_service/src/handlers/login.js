...
async function LoginClient(call, callback) {
  ...
  try {
    ...
    createEvent({
      ...
      action: "LOGIN",
      status: "SUCCESS",
      message: `Client logged in: ${email}`,
      timestamp: Date.now(),
    });
    ...
    console.error("❌ Login error (client):", msg, error.code || "");
    createEvent({
      actor_id: email,
      actor_type: "client",
      action: "LOGIN",
    ...
    callback(null, { token: "", role: "", success: false, message: msg });
  }
}
...
async function LoginUser(call, callback) {
  ...
  try {
    ...
    const msg = error.message || error.code || String(error);
    console.error("❌ Login error (user):", msg, error.code || "");
    createEvent({
      actor_id: email,
      actor_type: "user",
      action: "LOGIN",
